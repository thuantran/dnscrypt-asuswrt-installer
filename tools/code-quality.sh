...
if require_cmd shellcheck; then
	run_script_list_check 'ShellCheck static analysis' shellcheck -s sh --severity=warning
fi

if require_cmd shfmt; then
	if [ "${FIX}" -eq 1 ]; then
		run_script_list_check 'shfmt formatting update' shfmt -w -ln mksh -i 0 -ci
	else
		run_script_list_check 'shfmt formatting check' shfmt -d -ln mksh -i 0 -ci
	fi
fi
