...
- The latest dnscrypt-proxy start directly as privileged so it can bind privileged ports such as `127.0.1.1:53`, then shortly drops those privileges taking on the `user_name = 'nobody'` achieving the same state of nonroot.
